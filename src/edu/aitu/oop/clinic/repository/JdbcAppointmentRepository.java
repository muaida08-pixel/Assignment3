...
public class JdbcAppointmentRepository implements AppointmentRepository {
    ...
    public void save(Appointment appointment) {
        ...
        try (Connection conn = PostgresDB.getInstance().getConnection();
            ...
            stmt.setTimestamp(4, Timestamp.valueOf(LocalDateTime.of(appointment.getDate(), appointment.getTime())));
    ...
    public Appointment findById(Long id) {
        ...
        try (Connection conn = PostgresDB.getInstance().getConnection();
            ...
            stmt.setLong(1, id);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) return mapRowToAppointment(rs);
        } catch (SQLException e) { e.printStackTrace(); }
        return null;
    }
    ...
    private Appointment mapRowToAppointment(ResultSet rs) throws SQLException {
        Doctor doctor = new Doctor(rs.getLong("doc_id"), rs.getString("doc_name"), rs.getString("doc_spec"));
        Patient patient = new Patient(rs.getLong("pat_id"), rs.getString("pat_name"), rs.getString("pat_email"));
